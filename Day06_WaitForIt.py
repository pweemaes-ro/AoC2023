...
def get_interval_size(time_and_distance: tuple[int, int]) -> int:
	"""Return size of interval [r, s] with r the smallest integer x and s the
	largest integer satisfying equation -x^2 - time * x > distance."""

	time, distance = time_and_distance

	# Calculate the roots of -x^2 - time * x = distance.
	# abc formula: a = -1, b = -time, c = -distance.
	
	# discriminant = b * b - 4 * a * c
	#              = -time * -time - -4 * -1 * distance
	...
	# roots = (-b pm square_root(discriminaot) / 2a
	#       = (time pm square_root_of_discriminant) / -2
	# Since denominator < 0 and roots always positive, numerator < 0, so
	...
	# If float roots, smaller root should be rounded up, larger rounded down.
